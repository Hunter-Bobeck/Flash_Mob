...
public class Spin
{
	...
	triggers_subsafe_colloquial = {"spin", "twirl", "whirl", "pirouette"},
	...
	(
		...
		(
			...
			(
				new String[] {"Your "},
				Slash.triggers_colloquial_and_weapon,
				new String[] {" "},
	...
	public Spin(Scenario scenario, Player player)
	{
		Mob mob = scenario.encounter.mob;
		if (Array.containment(player.abilities, "spin") && !on_cooldown)
		{
			// removing cooldowns //
			...
			// damage rolling //
			int damage_pure = player.roll_damage_pure_weapon() + Random.integer_from(0, 10), damage;
		...
		else
		{
			...
			{
				Type.delay_line(Random.text_of(new String[] {"You haven't learned to whirl yet", "You need to train yourself to pirouette first"})+".");
