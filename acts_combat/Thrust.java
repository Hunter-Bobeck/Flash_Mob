...
public class Thrust
{
	...
	triggers_subsafe_colloquial = {"thrust", "lunge"},
	...
	triggers_nonsubsafe_colloquial = {"stab"},
	...
	(
		...
		(
			...
			(
				...
				new String[] {" "},
				new String[] {"misses", "misses", "parts the air", "misses its mark", "misses", "misses", "parts the air", "misses its mark", "wounds only the wind", "deals damage to the air"}
			)
		),
		new String[] {"."}
	);
	public Thrust(Scenario scenario, Player player)
	{
		Mob mob = scenario.encounter.mob;
		if (Array.containment(player.abilities, "thrust"))
		{
			// removing cooldowns //
			...
			else	// mob blocks partially or exactly; player's attack hits //
			{
				...
				// hit statement //
				if (damage > 0  &&  Random.whole() <= .2)		// player's attack hits critically //
				{
