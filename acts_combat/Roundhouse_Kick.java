...
public class Roundhouse_Kick
{
	public static final int experience_cost = 250;
    ...
    triggers_subsafe_uncolloquial = {"rotating kick", "spin boot", "spinning boot", "spin calcitrat", "spinning calcitrat"},
	
	triggers_subsafe_colloquial = {"roundhouse", "roundhouse kick", "spinning kick", "spin kick"},
	...
	public Roundhouse_Kick(Scenario scenario, Player player)
	{
		Mob mob = scenario.encounter.mob;
		if (Array.containment(player.abilities, "roundhouse kick") && !on_cooldown)
		{
			// removing cooldowns //
			...
			// damage rolling //
			int damage_pure = player.roll_damage_pure_unarmed() + Random.integer_from(1, Random.integer_from(14, 15)), damage;
			Wait.milliseconds(800);
			if (damage_pure < mob.block)		// mob overblocks; player's attack glances //
			{
				// glancing damage randomization //
				damage = Random.integer_of(new int[] {0, 1, 1, 2, 3});
		...
		else
		{
			...
			{
				Type.delay_line(Random.text_of(new String[] {"You haven't learned to execute a roundhouse kick yet", "You need to train yourself to do roundhouse kicks first"})+".");
