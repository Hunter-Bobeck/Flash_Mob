...
public class Magic_Missile
{
	public static final int experience_cost = 130;
    ...
    triggers_subsafe_uncolloquial = {"magic", "missile", "energy"},
	
	triggers_subsafe_colloquial_uncastpermuted = {"magic missile", "energy bolt"},
	...
	triggers_nonsubsafe_uncolloquial = {"mm"},
	...
	(
		...
		(
			...
			(
				...
				triggers_subsafe_colloquial_uncastpermuted,
				new String[] {" "},
				new String[] {"misses", "misses its target", "misses", "misses its target", "misses", "misses its target", "dematerializes upon casting", "fails to cast"}
			)
		),
		new String[] {"."}
	);
	public Magic_Missile(Scenario scenario, Player player)
	{
		Mob mob = scenario.encounter.mob;
		if (Array.containment(player.abilities, "magic missile"))
		{
			...
			if (damage_pure < mob.block)		// mob overblocks; player's attack glances //
			{
				...
				damage = Random.integer_of(new int[] {1, 1, 1, 1, 1});
		...
		else
		{
			if (player.mage)
			{
				Type.delay_line(Random.text_of(new String[] {"You don't know the magic missile spell", "You haven't learned to cast magic missile yet"})+". ", 25);
