...
public class Elbow_Strike
{
	public static final int experience_cost = 180;
    ...
    triggers_subsafe_uncolloquial = {"elbow", "clobber"},
	
	triggers_subsafe_colloquial = {"elbow strike", "elbow strike", "elbow jab", "clobber with elbow"},
	...
	public Elbow_Strike(Scenario scenario, Player player)
	{
		Mob mob = scenario.encounter.mob;
		if (Array.containment(player.abilities, "elbow strike") && !on_cooldown)
		{
			// removing cooldowns //
			...
			// damage rolling //
			int damage_pure = player.roll_damage_pure_unarmed() + Random.integer_from(1, 4), damage;
			Wait.milliseconds(800);
			if (damage_pure < mob.block)		// mob overblocks; player's attack glances //
			{
				// glancing damage randomization //
				damage = Random.integer_of(new int[] {0, 0, 0, 1, 2});
				...
				(
					...
					(
						...
						(
							...
							(
								new String[] {"hardly "},
								new String[] {"hit with enough force", "bring enough weight to bear"}
							)
						)
					)
				), new String[] {"."});
			...
			else	// mob blocks partially or exactly; player's attack hits //
			{
				...
				// hit statement //
				if (damage > 0  &&  Random.whole() <= .25)		// player's attack hits critically //
				{
		...
		else
		{
			...
			{
				Type.delay_line(Random.text_of(new String[] {"You haven't learned to execute an elbow strike yet", "You need to train yourself to do the elbow strike ability first"})+".");
