...
{
	public static final int experience_cost = 450;
    ...
    triggers_subsafe_uncolloquial = {"topdown"},
	
	triggers_subsafe_colloquial = {"overhead", "overhead", "overhead swing", "topdown swing", "split", "splitting overhead"},
	
	
	triggers_subsafe = Array.combine(triggers_subsafe_uncolloquial, triggers_subsafe_colloquial),
	
	
	triggers_nonsubsafe_uncolloquial = {"over", "crash"},
	...
	(
		...
		(
			...
			(
				...
				new String[] {" "},
				new String[] {"misses", "misses", "parts the air", "misses its mark", "misses", "misses", "parts the air", "misses its mark", "wounds only the wind", "splits a particle in the air", "cleaves the air", "whiffs", "whooshes down harmlessly"}
			)
		),
		new String[] {"."}
	);
	public Overhead(Scenario scenario, Player player)
	{
		Mob mob = scenario.encounter.mob;
		if (Array.containment(player.abilities, "overhead") && !on_cooldown)
		{
			// removing cooldowns //
			...
			// damage rolling //
			int damage_pure = player.roll_damage_pure_weapon(), damage;
			...
			else	// mob blocks partially or exactly; player's attack hits //
			{
				...
				// hit statement //
				if (damage > 0  &&  Random.whole() <= .77)		// player's attack hits critically //
				{
		...
		else
		{
			...
			{
				Type.delay_line(Random.text_of(new String[] {"You haven't learned to execute an overhead yet", "You need to train yourself to do topdown strikes first"})+".");
