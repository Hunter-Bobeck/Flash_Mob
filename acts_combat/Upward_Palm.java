...
public class Upward_Palm
{
	public static final int experience_cost = 650;
    ...
    triggers_subsafe_uncolloquial = {"upward hand", "upward-facing palm strike", "upward facing palm"},
	
	triggers_subsafe_colloquial = {"upward palm", "palm", "palm strike", "upward palm strike", "upward-facing palm"},
	...
	(
		...
		(
			...
			(
				...
				triggers_colloquial,
				new String[] {" "},
				new String[] {"misses", "misses", "hits only air", "had too little weight", "was not angled properly", "comes up short", "was too slow"}
			)
		),
		new String[] {"."}
	);
	public Upward_Palm(Scenario scenario, Player player)
	{
		Mob mob = scenario.encounter.mob;
		if (Array.containment(player.abilities, "upward palm") && !on_cooldown)
		{
			// removing cooldowns //
			...
			// damage rolling //
			int damage_pure = player.roll_damage_pure_unarmed() + Random.integer_from(1, 17), damage;
			Wait.milliseconds(800);
			if (damage_pure < mob.block)		// mob overblocks; player's attack glances //
			{
				// glancing damage randomization //
				damage = Random.integer_of(new int[] {0, 0, 1, 2, 3});
				...
				(
					...
					(
						...
						(
							...
							(
								new String[] {"hardly "},
								new String[] {"strike with enough force", "bring enough weight to bear"}
							)
						)
					)
				), new String[] {"."});
			...
			else	// mob blocks partially or exactly; player's attack hits //
			{
				...
				// hit statement //
				if (damage > 0  &&  Random.whole() <= .5122)		// player's attack hits critically //
				{
		...
		else
		{
			...
			{
				Type.delay_line(Random.text_of(new String[] {"You haven't learned to execute upward-facing palm yet", "You need to train yourself to do an upward palm strike first"})+".");
