...
public class Cut
{
    ...
    triggers_subsafe_uncolloquial = {"mince"},
	
	triggers_subsafe_colloquial = {"chop", "slice"},
	...
	triggers_nonsubsafe_colloquial = {"cut"},
	...
	(
		...
		(
			...
			(
				...
				new String[] {" "},
				new String[] {"misses", "misses", "hits only air", "comes up short", "comes too slowly", "misses", "misses", "hits only air", "comes up short", "comes too slowly", "whistles harmlessly", "whistles past harmlessly", "slices through the air", "minces a particle in the air"}
			)
		),
		new String[] {"."}
	);
	public Cut(Scenario scenario, Player player)
	{
		Mob mob = scenario.encounter.mob;
		if (Array.containment(player.abilities, "cut"))
		{
			// removing cooldowns //
			...
			if (damage_pure < mob.block)		// mob overblocks; player's attack glances //
			{
				...
				damage = Random.integer_of(new int[] {0, 1, 1, 1, 1});
			...
			else	// mob blocks partially or exactly; player's attack hits //
			{
				...
				// hit statement //
				if (damage > 0  &&  Random.whole() <= .1)		// player's attack hits critically //
				{
